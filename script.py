...
Run the installation command in a separate Colab cell:

!pip install -q esinet "mne>=1.7,<1.10" pandas seaborn matplotlib \
...
trans_file = find_first([
    ...
    os.path.join(CFG.mne_root, "**", "*-trans.fif"),
])
bem_file = find_first([
...
for seed_index, seed in enumerate(RUN["seeds"]):
    ...
    if CFG.train_convdip:
        ...
        try:
            convdip_model.save(str(OUT / f"convdip_seed_{seed}"))
    ...
    for condition, (physical_eeg, target_raw) in test_raw.items():
        ...
        cnn_model = trained.get("tikhonov_cnn")
        sparse_model = trained.get("sparse_graph")
        physics_model = trained.get("physics_gat")

        if cnn_model is None:
            raise RuntimeError(
                "Trained Tikhonov-CNN model is missing."
            )

        if sparse_model is None:
            raise RuntimeError(
                "Trained sparse-graph model is missing."
            )

        if physics_model is None:
            raise RuntimeError(
                "Trained Physics-GAT model is missing."
            )

        with torch.no_grad():
            cnn_output = cnn_model(source_tensor)

            predictions["tikhonov_cnn"] = (
                cnn_output.cpu().numpy()
                * target_scale
            )

            sparse_output = sparse_model(source_tensor)

            predictions["sparse_graph"] = (
                sparse_output.cpu().numpy()
                * target_scale
            )

            test_graph_dataset = SourceDataset(
                initial=initial,
            ...
            test_graph_loader = PyGDataLoader(
                test_graph_dataset,
                batch_size=RUN["batch_size"],
                shuffle=False,
                num_workers=0,
            ...
            for graph_batch in test_graph_loader:
                ...
                graph_output = physics_model(graph_batch)

                graph_outputs.append(
                    graph_output.cpu().numpy()
                )

            if not graph_outputs:
                raise RuntimeError(
                    "Physics-GAT produced no synthetic predictions."
                )
        ...
        if convdip_model is not None:
            predictions["convdip"] = convdip_synthetic(
                convdip_model,
                physical_eeg,
            )
        ...
        for algorithm, prediction in predictions.items():
            ...
            if prediction.shape != target_raw.shape:
                raise RuntimeError(
                    f"{algorithm} shape mismatch: "
                    f"prediction={prediction.shape}, "
                    f"target={target_raw.shape}"
                )

            for sample_index in range(len(target_raw)):
                row = {
                    "seed": seed,
                    "condition": condition,
                    "algorithm": algorithm,
                ...
                sample_result = metrics(
                    target_raw[sample_index],
                    prediction[sample_index],
                )

                row.update(sample_result)

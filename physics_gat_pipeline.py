...
@dataclass
class Config:
    run_mode: str = "full"  # smoke | pilot | full
...
class TemporalEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        # Input channel dimension is N_TIMES + N_CHANNELS due to lead field concatenation
        input_dim = N_TIMES + N_CHANNELS
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x has shape (batch_size * N_VERTICES, N_TIMES + N_CHANNELS)
...
class TikhonovTemporalCNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        batch, vertices, features = x.shape
        hidden = self.encoder(x.reshape(batch * vertices, features))
...
class SparseGraphNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        batch, vertices, features = x.shape
        hidden = self.encoder(x.reshape(batch * vertices, features)).reshape(batch, vertices, -1)
...
class PhysicsGAT(nn.Module):
    def __init__(self):
        ...
        # Introduce SAGPooling to drop less significant nodes based on attention weights
        self.pool = SAGPooling(CFG.hidden, ratio=0.5)
...
def reshape_batch(batch: Data) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    return (
        batch.x.reshape(count, N_VERTICES, N_TIMES + N_CHANNELS).to(DEVICE),
...
for seed_index, seed in enumerate(RUN["seeds"]):
    ...
    for condition, (physical_eeg, target_raw) in test_raw.items():
        ...
        # Concatenate lead field for non-GAT models too if they use it,
        # but here we construct the exact same feature map for Tikhonov and SparseGraph
        lead_field_np = LEAD_FIELD.T
        initial_features = np.concatenate([initial, np.repeat(lead_field_np[None], len(initial), axis=0)], axis=2)
        source_tensor = torch.from_numpy(initial_features).to(DEVICE)
    ...
    if CFG.run_real_eeg:
        ...
        for condition, evoked in evokeds.items():
            ...
            initial_features = np.concatenate([initial, LEAD_FIELD.T], axis=1)
            source_tensor = torch.from_numpy(initial_features[None]).to(DEVICE)
            predictions = {}
            ...
            gat_batch = next(iter(PyGDataLoader([
                Data(x=torch.from_numpy(initial_features), edge_index=edge_index,
                     edge_attr=edge_attr, pos=torch.from_numpy(COORDINATES_MM))
